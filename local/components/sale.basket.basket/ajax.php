...
if(!empty($request->get('basket')['coupon'])){
    session_start();
    $curl = curl_init();
    curl_setopt($curl, CURLOPT_URL, 'http://'.$_SERVER['HTTP_HOST'].'/local/api/widget/execute.php?code='.$request->get('basket')['coupon'].'&act=use');
